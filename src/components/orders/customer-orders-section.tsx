...
export function CustomerOrdersSection() {
  ...
  const fetchOrders = useCallback(async (filter: InvoiceFilter) => {
    ...
    try {
      const response = await fetch(`/api/customer/orders?filter=${filter}`)
  ...
  useEffect(() => {
    const loadOrders = async () => {
      await Promise.all([fetchOrders('recent'), fetchOrders('completed')])
  ...
  // Not linked to Zoho
  if (!recentOrders.linked && !completedOrders.linked) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>My Orders</CardTitle>
        ...
        <CardContent>
          <div className="flex items-start gap-3 text-muted-foreground">
            ...
            <p>{recentOrders.message || 'Order tracking is being set up for your account.'}</p>
          </div>
        </CardContent>
      </Card>
    )
